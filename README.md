...
离线链路：
Agent 模拟器 -> MySQL Source -> Sqoop -> HDFS Raw
...
├── scripts/              # Kafka、Sqoop、Spark 作业脚本
...
6. 启动实时模拟器。
7. 离线链路按顺序运行：离线模拟器、Sqoop、Clean Job、Batch Jobs、报告生成。

## 设计来源

本仓库根据《AgentScope 多智能体运行监测与效能分析平台设计说明书》初始化，当前目标是提供清晰、可扩展、可答辩的工程起点。后续开发优先级：

1. 打通 MySQL Source、Sqoop 和 HDFS Raw。
