...
*   **一键式自动化调度与数据注入**: 
    *   提供一键执行总控脚本，并附带 Crontab 定时调度示例；是否自动运行取决于生产环境是否已单独部署该调度，展示正在执行节点的霓虹高亮效果。
...
* **实时模式**（向 Kafka 持续注入流数据）：
  ...
  ```
* **离线数据生成与自动调度**：
  * **定时自动运行**：项目提供 **Crontab 定时任务，每天凌晨 02:00** 自动触发离线数据生成与计算 Pipeline 的示例配置；是否生效取决于生产环境是否已真正部署该任务：
    ```cron
